...
pro modis_calculate_toa_DCC,input_directory=input_directory
  compile_opt idl2
  ;input_directory='F:\MODIS_DCC\hdf' ;
  input_directory_GEO=input_directory+'-GEO'
  ;out_directory='F:\MODIS_DCC\tiff\cloutiff\'
  DestPath='H:\00data\MODIS\DCC\DCC_NAN\2019'
  DestPath1='H:\00data\MODIS\DCC\DCC_NAN\2019-GEO'
  ;文件日期 角度 匹配站点范围各个波段的toa均值 
  openw,lun,'H:\00data\MODIS\DCC\TOA\0.0\DCC_modis-0.0.txt',/get_lun,/append,width=500
...
;    ra_angle = (ra_angle le 180.0) * ra_angle + (ra_angle gt 180.0) * (360.0 - ra_angle)  ;相对方位角
    ...
    ;write_tiff,'F:\MODIS_DCC\tiff\basetiff\'+datetime+'TOA_base_DCC.tiff',DCC_TOA_ref_angle,planarconfig=2,compression=1,/float;,GEOTIFF=GEOTIFF
    ;云掩膜处理
    ...
    ;write_tiff,out_directory+datetime+'TOA_cloud_DCC-0.0.tiff',DCC_TOA_ref_angle,planarconfig=2,compression=1,/float;,GEOTIFF=GEOTIFF
...
;    DCC_angle_mean=[mean(DCC_TOA_ref_angle[*,*,-7]),mean(DCC_TOA_ref_angle[*,*,-6]),mean(DCC_TOA_ref_angle[*,*,-5]),mean(DCC_TOA_ref_angle[*,*,-4]),mean(DCC_TOA_ref_angle[*,*,-3])]
    ...
    data=[string(datetime),string(DCC_angle_mean),string(DCC_TOA_ref_mean),string(DCC_Data0064_size[4]),string(DCC_TOA_ref_count)]
