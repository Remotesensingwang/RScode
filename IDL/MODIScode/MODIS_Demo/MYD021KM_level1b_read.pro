...
pro MYD021KM_level1b_read,MYD021KMFile,imagedata,$
  ...
  if keyword_set(reflectance) then begin
    for layer_i=0,DN_band_data_size[3]-1 do begin
      imagedata[*,*,layer_i]=scales_band_data[layer_i]*(((DN_band_data[*,*,layer_i] gt 0) and (DN_band_data[*,*,layer_i] lt 32767))*DN_band_data[*,*,layer_i]-offsets_band_data[layer_i])    
    endfor
  endif
  ...
  if keyword_set(reflectance) and keyword_set(sz_angle) then begin
    ...
    for layer_i=0,3 do begin
      imagedata[*,*,layer_i]=(scales_band_data[layer_i]*(((DN_band_data[*,*,layer_i] gt 0) and (DN_band_data[*,*,layer_i] lt 32767))*DN_band_data[*,*,layer_i]-offsets_band_data[layer_i]))/cos(sz_angle*!dtor)    
    endfor      
  endif
